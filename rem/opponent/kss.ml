...
let () =
                ...
                begin
                  ...
                  | 'T' | 'o' | 'K' | 'J' | 'N' | '.' ->
                    ...
                    begin
                      ...
                      play color_orig turn board ; (*try-agan*)
                    end;
  ...
  and run turn pos board =
    ...
    *)
    ...
    begin if c0 > 7 || c1 > 7 then run turn None board
    end;
    Printf.printf "# %d %d\n%!" c0 c1;
    match pos with
    | Some (d0, d1) ->
        ...
        run turn None board
        *)
        play color turn board

    | None ->
      let (color, ca)  = Lists.access_n2 board c0 c1 in
      begin if ca = '_' then run turn None board end;
      (*
      begin
        match ca with
        | 'T' -> Printf.printf "# 0 +1 \n";
      ...
      *)
      ...
      Lists.print board;
      run turn (Some (c0, c1)) board
  in
  try run turn None board
